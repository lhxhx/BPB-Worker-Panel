...
let userID = "bd0524dd-ad6c-4260-b18a-d53454187e20";

// https://www.nslookup.io/domains/cdn.xn--b6gac.eu.org/dns-records/
// https://www.nslookup.io/domains/cdn-all.xn--b6gac.eu.org/dns-records/
const proxyIPs = [
  "cdn.xn--b6gac.eu.org", "cdn-all.xn--b6gac.eu.org", "edgetunnel.anycast.eu.org",
    //   "199.15.78.127",
    //   "103.200.112.121",
    //   "38.47.103.34",
    //   "8.217.28.51",
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        ...
        switch (url.pathname) {
          ...
          case `/warpsub/${userID}`:
            ...
            return new Response(`${JSON.stringify(wowConfig, null, 4)}`, {
              status: 200,
            });
          ...
          case "/panel":
            ...
            const proxySettings = await env.bpb.get("proxySettings", {
              type: "json",
            });
          ...
          case "/login":
            ...
            if (request.method === "POST") {
              ...
              if (password === savedPass) {
                ...
                const cookieHeader = `jwtToken=${jwtToken}; HttpOnly; Secure; Max-Age=${
                  7 * 24 * 60 * 60
...
async function handleTCPOutBound(
  ...
  async function retry() {
    ...
    const tcpSocket = await connectAndWrite(
      panelProxyIP || proxyIP || addressRemote,
      portRemote
    );
    tcpSocket.closed
      .catch((error) => {
...
const getNormalConfigs = async (env, hostName, client) => {
  ...
  try {
    ...
    console.log(error);
    throw new Error(
      `An error occurred while getting normal configs - ${error}`
    );
  }
  ...
  ports.forEach((port) => {
    Addresses.forEach((addr, index) => {
      vlessWsTls +=
        "vless" +
        `://${userID}@${addr}:${port}?encryption=none&type=ws&host=${randomUpperCase(
          hostName
        )}${
          defaultHttpsPorts.includes(port)
            ? `&security=tls&sni=${randomUpperCase(
                hostName
              )}&fp=randomized&alpn=${
...
const buildProxyOutbound = async (proxyParams) => {
  ...
  switch (security) {
    case "tls":
      ...
      proxyOutbound.streamSettings.tlsSettings.alpn = alpn
        ? alpn?.split(",")
        : [];
  ...
  switch (type) {
    case "tcp":
      ...
      if (headerType === "http") {
        proxyOutbound.streamSettings.tcpSettings.header.request.headers.Host =
          host?.split(",");
        proxyOutbound.streamSettings.tcpSettings.header.request.path =
          path?.split(",");
      }
...
const buildWorkerLessConfig = async (env, client) => {
  ...
  try {
    ...
    console.log(error);
    throw new Error(
      `An error occurred while generating WorkerLess config - ${error}`
    );
  }
  ...
  fragConfig.dns = await buildDNSObject(
    remoteDNS,
    localDNS,
    blockAds,
    bypassIran,
  ...
  fragConfig.routing.rules = buildRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
...
const getFragmentConfigs = async (env, hostName, client) => {
  ...
  try {
    ...
    console.log(error);
    throw new Error(
      `An error occurred while getting fragment configs - ${error}`
    );
  }
  ...
  for (let portIndex in ports.filter((port) =>
    defaultHttpsPorts.includes(port)
  )) {
  ...
  bestPing.dns = await buildDNSObject(
    remoteDNS,
    localDNS,
    blockAds,
    bypassIran,
  ...
  if (proxyOutbound) {
    ...
    bestPing.routing.rules = buildRoutingRules(
      localDNS,
      blockAds,
      bypassIran,
      blockPorn,
  ...
  bestFragment.dns = await buildDNSObject(
    remoteDNS,
    localDNS,
    blockAds,
    bypassIran,
  ...
  let bestFragmentOutbounds = structuredClone([
    { ...outbounds[0] },
    { ...outbounds[1] },
  ]);
  ...
  if (proxyOutbound) {
    ...
    bestFragment.outbounds = [
      bestFragmentOutbounds[0],
      bestFragmentOutbounds[1],
      ...bestFragment.outbounds,
    ];
    bestFragment.routing.rules = buildRoutingRules(
      localDNS,
      blockAds,
      bypassIran,
      blockPorn,
...
const getSingboxConfig = async (env, hostName) => {
  ...
  let outboundDomains = [];
  const domainRegex =
    /^(?!:\/\/)([a-zA-Z0-9-]{1,63}\.)*[a-zA-Z0-9][a-zA-Z0-9-]{0,62}\.[a-zA-Z]{2,11}$/;
  ...
  try {
    ...
    console.log(error);
    throw new Error(
      `An error occurred while getting sing-box configs - ${error}`
    );
  }
  ...
  ports.forEach((port) => {
    Addresses.forEach((addr, index) => {
      ...
      if (domainRegex.test(outbound.server))
        outboundDomains.push(outbound.server);
    });
  });
...
const getWarpConfigs = async (env, client) => {
  ...
  const { xray: xrayWarpOutbounds, singbox: singboxWarpOutbounds } =
    await buildWarpOutbounds(
      remoteDNS,
      localDNS,
      blockAds,
      bypassIran,
  ...
  const { xray: xrayWoWOutbounds, singbox: singboxWoWOutbounds } =
    await buildWoWOutbounds(
      remoteDNS,
      localDNS,
      blockAds,
      bypassIran,
  ...
  xrayWarpConfig.dns = await buildDNSObject(
    remoteDNS,
    localDNS,
    blockAds,
    bypassIran,
  ...
  xrayWarpConfig.routing.rules = buildRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
  ...
  xrayWarpBestPing.routing.rules = buildRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
  ...
  xrayWoWConfigTemp.dns = await buildDNSObject(
    remoteDNS,
    localDNS,
    blockAds,
    bypassIran,
  ...
  xrayWoWConfigTemp.routing.rules = buildRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
  ...
  xrayWoWOutbounds.forEach((outbound, index) => {
    if (outbound.tag.includes("warp-out")) {
      ...
      xrayWoWConfig.outbounds = [
        { ...xrayWoWOutbounds[index] },
        { ...xrayWoWOutbounds[index + 1] },
        ...xrayWoWConfig.outbounds,
      ];
      xrayWoWConfig.routing.rules[
        xrayWoWConfig.routing.rules.length - 1
      ].outboundTag = outbound.tag;
      xrayWarpConfigs.push(xrayWoWConfig);
    }
  });

  xrayWarpBestPing.outbounds = [
    ...xrayWarpOutbounds,
    ...xrayWarpBestPing.outbounds,
  ];
  ...
  singboxWoWOutbounds.forEach((outbound, index) => {
    if (outbound.tag.includes("WoW")) {
      singboxWarpConfig.outbounds.push(
        singboxWoWOutbounds[index],
        singboxWoWOutbounds[index + 1]
      );
      singboxWarpConfig.outbounds[0].outbounds.push(outbound.tag);
      if (domainRegex.test(outbound.server))
        outboundDomains.push(outbound.server);
  ...
  return client === "singbox" ? singboxWarpConfig : xrayWarpConfigs;
};

const buildWarpOutbounds = async (
  remoteDNS,
  localDNS,
  blockAds,
  bypassIran,
  ...
  xrayOutboundTemp.settings.peers[0].publicKey =
    wgConfig.account.config.peers[0].public_key;
  xrayOutboundTemp.settings.reserved = base64ToDecimal(
    wgConfig.account.config.client_id
  );
  ...
  warpEndpoints.split(",").forEach((endpoint, index) => {
    ...
    singboxOutbounds.push({
      ...singboxOutbound,
      server: endpoint.includes("[")
        ? endpoint.match(ipv6Regex)[1]
        : endpoint.split(":")[0],
      server_port: endpoint.includes("[")
        ? +endpoint.match(portRegex)[0]
        : +endpoint.split(":")[1],
      tag: `💦 Warp ${index + 1} 🇮🇷`,
    });
  });
...
const buildWoWOutbounds = async (
  ...
  wowEndpoint.split(",").forEach((endpoint, index) => {
    for (let i = 0; i < 2; i++) {
      ...
      xrayOutbound.settings.peers[0].endpoint = endpoint;
      xrayOutbound.settings.peers[0].publicKey =
        wgConfigs[i].account.config.peers[0].public_key;
      xrayOutbound.settings.reserved = base64ToDecimal(
        wgConfigs[i].account.config.client_id
      );
      ...
      if (i === 1) {
        ...
        xrayOutbound.streamSettings.sockopt.dialerProxy = `warp-ir_${
          index + 1
        }`;
      }
      ...
      singboxOutbound.server = endpoint.includes("[")
        ? endpoint.match(ipv6Regex)[1]
        : endpoint.split(":")[0];
      singboxOutbound.server_port = endpoint.includes("[")
        ? +endpoint.match(portRegex)[0]
        : +endpoint.split(":")[1];
      singboxOutbound.peer_public_key =
        wgConfigs[i].account.config.peers[0].public_key;
...
const buildDNSObject = async (
  ...
  if (isWorkerLess) {
    ...
    const resolvedCLDomain = await resolveDNS(
...
const buildRoutingRules = (
  ...
  if (blockAds || blockPorn) {
    ...
    blockAds &&
      rule.domain.push("geosite:category-ads-all", "geosite:category-ads-ir");
...
const updateDataset = async (env, Settings) => {
  ...
  try {
    ...
    console.log(error);
    throw new Error(
      `An error occurred while getting current values - ${error}`
    );
  }
  ...
  const proxySettings = {
    remoteDNS: Settings
      ? Settings.get("remoteDNS")
      : currentProxySettings?.remoteDNS || "https://94.140.14.14/dns-query",
    localDNS: Settings
      ? Settings.get("localDNS")
      : currentProxySettings?.localDNS || "8.8.8.8",
    lengthMin: Settings
      ? Settings.get("fragmentLengthMin")
      : currentProxySettings?.lengthMin || "100",
    lengthMax: Settings
      ? Settings.get("fragmentLengthMax")
      : currentProxySettings?.lengthMax || "200",
    intervalMin: Settings
      ? Settings.get("fragmentIntervalMin")
      : currentProxySettings?.intervalMin || "5",
    intervalMax: Settings
      ? Settings.get("fragmentIntervalMax")
      : currentProxySettings?.intervalMax || "10",
    blockAds: Settings
      ? Settings.get("block-ads")
      : currentProxySettings?.blockAds || false,
    bypassIran: Settings
      ? Settings.get("bypass-iran")
      : currentProxySettings?.bypassIran || false,
    blockPorn: Settings
      ? Settings.get("block-porn")
      : currentProxySettings?.blockPorn || false,
    bypassLAN: Settings
      ? Settings.get("bypass-lan")
      : currentProxySettings?.bypassLAN || false,
    cleanIPs: Settings
      ? Settings.get("cleanIPs")?.replaceAll(" ", "")
      : currentProxySettings?.cleanIPs || "",
    proxyIP: Settings
      ? Settings.get("proxyIP")
      : currentProxySettings?.proxyIP || "",
    ports: Settings
      ? Settings.getAll("ports[]")
      : currentProxySettings?.ports || ["443"],
    outProxy: Settings ? vlessConfig : currentProxySettings?.outProxy || "",
    outProxyParams: vlessConfig
      ? await extractVlessParams(vlessConfig)
      : currentProxySettings?.outProxyParams || "",
    wowEndpoint: Settings
      ? Settings.get("wowEndpoint")?.replaceAll(" ", "")
      : currentProxySettings?.wowEndpoint || "engage.cloudflareclient.com:2408",
    warpEndpoints: Settings
      ? Settings.get("warpEndpoints")?.replaceAll(" ", "")
      : currentProxySettings?.warpEndpoints ||
    ...
    [...defaultHttpPorts, ...defaultHttpsPorts].forEach((port) => {
      ...
      let portBlock = `
                <div class="routing" style="grid-template-columns: 1fr 2fr; margin-right: 10px;">
                    <input type="checkbox" id=${id} name=${port} onchange="handlePortChange(event)" value="true" ${
        ports.includes(port) ? "checked" : ""
      }>
                    <label style="margin-bottom: 3px;" for=${id}>${port}</label>
                </div>`;
      defaultHttpPorts.includes(port)
  ...
  const html = `
		...
		<div class="form-container">
			...
			<form id="configForm">
				...
				<div class="form-control" style="margin-bottom: 20px;">			
                    <div class="routing">
                        <input type="checkbox" id="block-ads" name="block-ads" style="margin: 0; grid-column: 2;" value="true" ${
                          blockAds ? "checked" : ""
                        }>
                        <label for="block-ads">Block Ads.</label>
                    </div>
                    <div class="routing">
						<input type="checkbox" id="bypass-iran" name="bypass-iran" style="margin: 0; grid-column: 2;" value="true" ${
              bypassIran ? "checked" : ""
            }>
                        <label for="bypass-iran">Bypass Iran</label>
					</div>
                    <div class="routing">
						<input type="checkbox" id="block-porn" name="block-porn" style="margin: 0; grid-column: 2;" value="true" ${
              blockPorn ? "checked" : ""
            }>
                        <label for="block-porn">Block Porn</label>
					</div>
                    <div class="routing">
						<input type="checkbox" id="bypass-lan" name="bypass-lan" style="margin: 0; grid-column: 2;" value="true" ${
              bypassLAN ? "checked" : ""
            }>
                ...
                <div class="table-container">
                    <table id="frag-sub-table">
                        ...
                        <tr>
                            ...
                            <td style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr;">${
                              (await buildPortsBlock()).httpsPortsBlock
                            }</td>    
                        </tr>
                        ${
                          hostName.includes("pages.dev")
                            ...
                            <td style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr;">${
                              (await buildPortsBlock()).httpPortsBlock
                            }</td>    
                        </tr>`
                        }        
                    </table>
	...
	<script>
        ...
        let activeHttpsPortsNo = ${
          ports.filter((port) => defaultHttpsPorts.includes(port)).length
        };
